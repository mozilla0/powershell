...
ForEach ($line in $aResult | Select -Skip ($indexHeader+1)) {
    $aPermission = $line.Split("|") # split in different columns
    $a = New-Object -TypeName PSObject
    $a | Add-Member -NotePropertyName Permission -NotePropertyValue $aPermission[1]
    $a | Add-Member -NotePropertyName Scope -NotePropertyValue $aPermission[2]
    $a | Add-Member -NotePropertyName Id -NotePropertyValue $aPermission[3]
    $hPermissions += $a
}
